<?php
	...
	function getSalesTotal($orderID) {
		...
		$query = "SELECT total FROM tbl_order WHERE status<>'cancelled' orderID = ".$orderID;
	...
	function getSalesGrandTotal($duration) {
		...
		if ($duration == "ALLTIME") {
			$query = "
					...
					FROM tbl_order WHERE status<>'cancelled'
					";
		}
			...
			$query = "
					...
					WHERE status<>'cancelled' AND order_date > DATE_SUB(NOW(), INTERVAL 1 ".$duration.")
					";
		}
