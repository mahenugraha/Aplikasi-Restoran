			...
			if ($menuItemResult->num_rows > 0) {
				if ($menuItemRow = $menuItemResult->fetch_array(MYSQLI_ASSOC)) {
					...
					<tr>
						...
						<td>". number_format($menuItemRow['price'], 0, ',', '.')."</td>
						<td><input type = 'number' required='required' min='1' max='50' name = 'itemqty[]' width='10px' class='form-control' value ='".$quantity."' readonly/></td>
						<td>" . number_format(($menuItemRow['price'] * $quantity), 0, ',', '.') . "</td>
